...
<script type="text/javascript">
            function createNewQuestion(){
                window.location.href="{{route('checkin.questions.create-edit')}}";
            }
    $(function() {
            ...
            $('body').on('click', '.view_record', function() {
                ...
                form_body.empty();
                $.post('{{ route('checkin.questions.details') }}', {
                    _token: '{{ csrf_token() }}',
                    id
                }, function(d) {
                    form_body.html(d);
            ...
            let table = $('#users_table').DataTable({
                ...
                lenghtChange:false,
                ajax: {
                    url: "{{ route('checkin.questions.list') }}",
                },
                columns: [{
                    data: 'question'
                },
            ...
            $('body').on('click', '.delete_record', function() {
                ...
                Swal.fire({
                    ...
                    if(data.isConfirmed==true){
                        $.post('{{ route('checkin.questions.delete') }}', {_token: '{{ csrf_token() }}', id}, function (d) {
                            if(d.success==true){
