            ...
            <div class="card-header border-0 pt-6">
                <div class="card-title">
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        class="form-control form-control-solid w-250px ps-14" placeholder="Search Movement Pattern" />
                    </div>
                </div>
...
<script type="text/javascript">
    $(function() {
        ...
        let table = $('#users_table').DataTable({
            ...
            lenghtChange:false,
            ajax: {
                url: "{{ route('setting.movementpattern.list') }}",
            },
            columns: [{
                data: 'name'
            },
        ...
        $('.create_new_record').on('click', function() {
            form_body.empty();
            $.post('{{ route('setting.movementpattern.details') }}', {
                _token: '{{ csrf_token() }}'
            }, function(d) {
                form_body.html(d);
            });
        ...
        $(document).on("submit", "form", function(event) {
            ...
            $('body').on('click', '.delete_record', function() {
                ...
                Swal.fire({
                    html: `Are you sure you want to delete this setting`,
                    icon: "info",
                    ...
                    if(data.isConfirmed==true){
                        $.post('{{ route('setting.movementpattern.delete') }}', {_token: '{{ csrf_token() }}', id}, function (d) {
                            if(d.success==true){
                                toastr.success(d.msg);
                                reloadTable();
                            }
