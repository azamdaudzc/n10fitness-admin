            ...
            <div class="card-header border-0 pt-6">
                <div class="card-title">
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        class="form-control form-control-solid w-250px ps-14" placeholder="Search Muscle" />
                    </div>
                </div>
...
<script type="text/javascript">
    $(function() {
        ...
        $('.create_new_record').on('click', function() {
            form_body.empty();
            $.post('{{ route('setting.muscle.details') }}', {
                _token: '{{ csrf_token() }}'
            }, function(d) {
                form_body.html(d);
            });
            ...
            $('body').on('click', '.delete_record', function() {
                ...
                Swal.fire({
                    ...
                    if(data.isConfirmed==true){
                        $.post('{{ route('setting.muscle.delete') }}', {_token: '{{ csrf_token() }}', id}, function (d) {
                            if(d.success==true){
                                toastr.success(d.msg);
                                reloadTable();
                            }
