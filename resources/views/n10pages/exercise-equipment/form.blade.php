    ...
    <div class="py-5">
           ...
           {{ Form::label('name', 'Name', array('class' => 'form-label')) }}
            {{ Form::text('name', $exerciseEquipment->name, ['class' => 'form-control' . ($errors->has('name') ? ' is-invalid' : ''), 'placeholder' => 'Name']) }}
      ...
      $.ajax({
        type:'{{$type}}',
         ...
         success:function(data){
            window.location.replace("{{route('exercise-equipments.index')}}");
